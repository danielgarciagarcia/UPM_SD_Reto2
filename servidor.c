...
/*
 * Receives a Linux group name and a RPC service request structure, and returns a data structure containing the GID of
 * the group as showed in the /etc/group file, as well as a dynamic-size vector of UIDs of all the users who are a
 * member of that group. If the group does not exist, the GID contained in the resulting data structures is set to -1.
 */
get_guid_reply *get_guid_1_svc(char **argp, struct svc_req *rqstp) {
    static get_guid_reply result;

    // Ensures the static structure result gets recursively freed after the RPC operation.
...
/*
 * Receives a Linux' user UID and a RPC service request structure, and returns a data structure containing the username
 * of that user, as well as a linked list of the group names of all the groups the user is member of. The data structure
 * also contains a boolean value indicating if the user exists or not.
 */
